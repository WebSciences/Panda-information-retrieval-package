 ...
 */
class ZcompressInputStream extends FilterInputStream {
    /**
     * @param is
     ...
     */
    public ZcompressInputStream(InputStream is) throws IOException {
        super(is);
        parse_header();
    }
    ...
    public synchronized int read(byte[] buf, int off, int len)
        ...
        /*
         * Using local copies of various variables speeds things up by as much
         * as 30% !
         */
        ...
        main_loop: do {
            ...
            int bit_in = (got > 0) ? (end - end % l_n_bits) << 3 : (end << 3)
                    - (l_n_bits - 1);
            ...
            while (l_bit_pos < bit_in) {
                ...
                if (l_free_ent > l_maxcode) {
                    ...
                    l_bit_pos = (l_bit_pos - 1) + n_bytes
                            - (l_bit_pos - 1 + n_bytes) % n_bytes;
                    ...
                    if (debug)
                        System.err
                                .println("Code-width expanded to " + l_n_bits);
                ...
                int pos = l_bit_pos >> 3;
                int code = (((l_data[pos] & 0xFF)
                        | ((l_data[pos + 1] & 0xFF) << 8) | ((l_data[pos + 2] & 0xFF) << 16)) >> (l_bit_pos & 0x7))
                        & l_bitmask;
                l_bit_pos += l_n_bits;
