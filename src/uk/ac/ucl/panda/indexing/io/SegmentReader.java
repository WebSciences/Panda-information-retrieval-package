...
class SegmentReader extends DirectoryIndexReader {
  ...
  static {
    try {
      String name =
        System.getProperty("uk.ac.ucl.panda.indexing.io.SegmentReader.class",
      ...
      if (!normsUpToDate) {
        ...
        for (int i = 0; i < fieldNormsChanged.length; i++) {
          ...
          if (!fieldNormsChanged[i]) {
            String curField = fieldInfos.fieldInfo(i).getName();
      ...
      if (clone.singleNormStream == null) {
        for (int i = 0; i < fieldInfos.size(); i++) {
          ...
          if (fi.isIndexed() && !fi.getOmitNorms()) {
            ...
            String fileName = si.getNormFileName(fi.getNum());
            if (si.hasSeparateNorms(fi.getNum())) {
              continue;
            }  
  ...
  static byte[] createFakeNorms(int size) {
    ...
    Arrays.fill(ones, Normalise.encodeNorm(1.0f));
    return ones;
  }
  ...
  private void openNorms(Directory cfsDir, int readBufferSize) throws IOException {
    ...
    for (int i = 0; i < fieldInfos.size(); i++) {
      FieldInfo fi = fieldInfos.fieldInfo(i);
      if (norms.containsKey(fi.getName())) {
      ...
      if (fi.isIndexed() && !fi.getOmitNorms()) {
        ...
        norms.put(fi.getName(), new Norm(normInput, singleNormFile, fi.getNum(), normSeek));
  ...
  public TermFreqVector getTermFreqVector(int docNumber, String field) throws IOException {
    ...
    if (fi == null || !fi.storeTermVector || termVectorsReaderOrig == null){
     //    System.out.println("aaaaa is null");
        return null;
    }
    TermVectorsReader termVectorsReader = getTermVectorsReader();
    if (termVectorsReader == null){
     //   System.out.println("bbbb is null");
        return null;
    }
     //System.out.println("cccc is null");
