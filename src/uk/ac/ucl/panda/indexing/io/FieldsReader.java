...
import uk.ac.ucl.panda.utility.structure.AbstractField;
...
public final class FieldsReader {
  ...
  private void addFieldLazy(Document doc, FieldInfo fi, boolean binary, boolean compressed, boolean tokenize) throws IOException {
    if (binary == true) {
      ...
      if (compressed) {
        ...
        doc.add(new LazyField(fi.getName(), Field.Store.COMPRESS, toRead, pointer));
      } else {
        //was: doc.add(new Fieldable(fi.name, b, Fieldable.Store.YES));
        doc.add(new LazyField(fi.getName(), Field.Store.YES, toRead, pointer));
      }
      ...
      if (compressed) {
        ...
        int toRead = fieldsStream.readVInt();
        long pointer = fieldsStream.getFilePointer();
        f = new LazyField(fi.getName(), store, toRead, pointer);
        ...
        f = new LazyField(fi.getName(), store, index, termVector, length, pointer);
        f.setOmitNorms(fi.getOmitNorms());
      }
      doc.add(f);
    }
  ...
  final static class FieldForMerge extends AbstractField {
    ...
    public FieldForMerge(Object value, FieldInfo fi, boolean binary, boolean compressed, boolean tokenize) {
      ...
      this.name = fi.getName().intern();
      this.isIndexed = fi.isIndexed();
      this.omitNorms = fi.getOmitNorms();          
